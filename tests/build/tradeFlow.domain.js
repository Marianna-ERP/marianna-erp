...
exports.shipmentTradeDirection = exports.TRADE_DIRECTIONS = exports.composePOFlow = exports.poDirectFromSOs = exports.dispositionFromSO = exports.handoverSentence = exports.namedPlacePoolForIncoterm = exports.MOVEMENT_LABELS = exports.movementFromEnds = exports.isEUCountry = exports.handoverPointForIncoterm = exports.handoverTextForIncoterm = exports.reconcilePOFlow = exports.CARGO_PLANS = exports.HANDOVER_POINTS = exports.TRADE_MOVEMENTS = exports.isDirectCargoPlan = exports.structToFlow = exports.flowToStruct = void 0;
// Legacy flow key → structured fields (for existing POs/lots).
const FLOW_TO_STRUCT = {
...
function flowToStruct(flow) {
    return FLOW_TO_STRUCT[flow] || null;
}
exports.flowToStruct = flowToStruct;
/**
 ...
 */
function structToFlow(s) {
    if (!s || !s.tradeMovement)
        return "";
    ...
    if (s.tradeMovement === "EXPORT") {
        if (pickup)
            return "EXP_EXWS";
        if (toPort)
            return s.handoverPoint === "dest_port" || s.requiresSea ? "EXP_CIF" : "EXP_FOB";
        if (direct)
            return s.crossBorder ? "EXP_DDP_XEU" : "EXP_DDP_EU";
        return "EXP_CIF";
    }
    // IMPORT
    if (inc === "DDP" || inc === "DAP")
        return direct ? "IMP_DDP_DIR" : "IMP_DDP_WH";
    if (inc === "CIF" || inc === "CFR")
        return direct ? "IMP_CIF_DIR" : "IMP_CIF_WH";
    // EXW/FCA/FOB purchase
    return direct ? "IMP_EXWS_DIR" : "IMP_EXWS_WH";
}
exports.structToFlow = structToFlow;
/** Round-trip stability check used by tests: struct→flow→struct preserves the essentials. */
function isDirectCargoPlan(s) {
    return (s === null || s === void 0 ? void 0 : s.cargoPlan) === "DIRECT_TO_CLIENT";
}
exports.isDirectCargoPlan = isDirectCargoPlan;
exports.TRADE_MOVEMENTS = [
    { code: "IMPORT", label: "Import" },
    { code: "EXPORT", label: "Export" },
];
exports.HANDOVER_POINTS = [
...
function reconcilePOFlow(po) {
    ...
    if (s)
        return { ...po, ...s, directFlow: s.cargoPlan === "DIRECT_TO_CLIENT" };
    return po;
}
exports.reconcilePOFlow = reconcilePOFlow;
// BP-56 / FB-4: incoterm-specific handover wording (derived, shown read-only).
function handoverTextForIncoterm(incoterm, tradeMovement) {
...
exports.handoverTextForIncoterm = handoverTextForIncoterm;
// Derive the physical handover point code from the incoterm (BP-56).
function handoverPointForIncoterm(incoterm) {
    const t = String(incoterm || "").toUpperCase();
    if (t === "EXW")
        return "supplier";
    if (t === "FCA")
...
exports.handoverPointForIncoterm = handoverPointForIncoterm;
...
function movementFromEnds(originInEU, placeInEU) {
    if (!originInEU && placeInEU)
        return "IMPORT";
    if (originInEU && !placeInEU)
        return "EXPORT";
    if (originInEU && placeInEU)
...
exports.movementFromEnds = movementFromEnds;
exports.MOVEMENT_LABELS = {
...
function namedPlacePoolForIncoterm(incoterm) {
    const t = String(incoterm || "").toUpperCase();
    if (t === "EXW" || t === "FCA")
        return { types: ["SUPPLIER"], label: "Pickup place (supplier site)" };
    if (t === "FOB")
        return { types: ["PORT"], label: "Port of loading" };
    if (t === "CFR" || t === "CIF")
        return { types: ["PORT"], label: "Port of discharge" };
    if (t === "DAP" || t === "DAT")
        return { types: ["CLIENT", "OWN"], label: "Delivery place" };
    if (t === "DDP")
        return { types: ["OWN", "CLIENT"], label: "Delivered to (our address)" };
    return { types: ["SUPPLIER", "PORT", "OWN", "CLIENT"], label: "Place (set the incoterm first)" };
}
exports.namedPlacePoolForIncoterm = namedPlacePoolForIncoterm;
/** One contractual sentence: responsibilities + the named place, in words. */
function handoverSentence(incoterm, placeName) {
    const t = String(incoterm || "").toUpperCase();
...
exports.handoverSentence = handoverSentence;
// ── Phase B (BP-57): the SALE owns disposition ───────────────────────────────
/** A sales order's disposition, derived from its sell terms. */
function dispositionFromSO(so) {
    const ic = String((so === null || so === void 0 ? void 0 : so.sellIncoterm) || "").toUpperCase();
    if (ic === "EXW")
        return "CLIENT_PICKUP";
    if (ic === "DAP" || ic === "DDP" || ic === "DAT")
        return "DIRECT_TO_CLIENT";
    if (ic === "CIF" || ic === "CFR" || ic === "FOB" || ic === "FCA")
        return "TO_PORT";
    return "OUR_WAREHOUSE";
}
exports.dispositionFromSO = dispositionFromSO;
...
function poDirectFromSOs(po, orders) {
    return (orders || []).some((o) => {
        if (!o || o.status === "Cancelled" || o.status === "Draft")
            return false;
        const sources = (o.items || []).some((it) => it.sourceType === "PO" && it.sourceRef === po.number);
        if (!sources)
            return false;
...
exports.composePOFlow = composePOFlow;
...
exports.TRADE_DIRECTIONS = ["IMPORT", "EXPORT", "INTRA_EU", "CROSS_TRADE"];
function shipmentTradeDirection(shipment, po) {
    if ((shipment === null || shipment === void 0 ? void 0 : shipment.tradeDirection) && exports.MOVEMENT_LABELS[shipment.tradeDirection])
        return shipment.tradeDirection;
    if ((po === null || po === void 0 ? void 0 : po.tradeMovement) && exports.MOVEMENT_LABELS[po.tradeMovement])
        return po.tradeMovement;
    if (String((po === null || po === void 0 ? void 0 : po.flow) || "").startsWith("EXP"))
        return "EXPORT";
    return "IMPORT";
}
exports.shipmentTradeDirection = shipmentTradeDirection;
