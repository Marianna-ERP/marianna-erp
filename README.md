...
│   ├── Dashboard.tsx           ← Phase 1 dashboard (6 KPI cards inc. margin)
│   ├── Contacts.tsx            ← integrated counterparty directory
│   ├── PurchaseOrders.tsx      ← integrated
│   ├── Inventory.tsx           ← integrated (reads live SOs from shell)
│   ├── SalesOrders.tsx         ← integrated (P/L card injected into detail)
│   ├── Shipments.tsx           ← integrated logistics / transport module
│   ├── Finance.tsx             ← V4 NEW: aggregate P/L analytics module
│   ├── SOMarginCard.tsx        ← V4 NEW: per-SO inline P/L card
│   ├── marginCalculations.ts   ← V4 NEW: pure P/L math engine
│   ├── Settings.tsx            ← Export/Import JSON, Reset to demo
│   ├── useLocalStoredState.ts  ← localStorage persistence hook
...
This version adds profitability tracking. Open any SO → see a P/L card at the top with revenue, COGS, direct logistics costs, and margin (with Forecast/Actual toggle). Click the new Finance tab in the top nav → aggregate analytics across all SOs (top clients by margin, top products, monthly trend, pipeline vs delivered). The Dashboard now has a 6th KPI: "Margin this month".

See `V4_CHANGELOG.md` for the full list of changes.
See `DEPLOY_GUIDE.md` for the GitHub Desktop + Vercel deployment walkthrough.
