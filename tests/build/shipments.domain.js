...
exports.normalizeCustoms = exports.nextShipmentAction = exports.canonicalStatus = exports.SHIPMENT_LIFECYCLE = exports.appendSourceGoods = exports.buildCollectionShipment = exports.nextShipmentNumberPure = exports.findLotForSOLine = exports.responsibilityForPOShipment = exports.postShipmentToLots = exports.derivePurpose = void 0;
const salesOrders_domain_1 = require("./salesOrders.domain");
Object.defineProperty(exports, "findLotForSOLine", { enumerable: true, get: function () { return salesOrders_domain_1.findLotForSOLine; } });
function num(v) {
    const x = parseFloat(String(v !== null && v !== void 0 ? v : "").replace(",", "."));
    return isFinite(x) ? x : 0;
}
function derivePurpose(sh) {
    const p = String((sh === null || sh === void 0 ? void 0 : sh.purpose) || "").toUpperCase();
    if (p === "INBOUND" || p === "OUTBOUND" || p === "TRANSFER" || p === "RETURN")
        return p;
    // Legacy values + legacy inference (pre-Batch-3 shipments in stored data):
    if (p === "SO_DELIVERY" || ((sh === null || sh === void 0 ? void 0 : sh.soRefs) || []).length > 0)
        return "OUTBOUND";
    if (p === "TRANSFER_INTERNAL")
        return "TRANSFER";
    if (p === "RETURN_INBOUND")
        return "RETURN";
...
function postShipmentToLots(sh, lots, deps) {
    ...
    const nextLots = (lots || []).map(lot => {
        ...
        if (hasMovement)
            return lot;
        const qty = relatedGoods.reduce((s, g) => s + num(g.qtyKg), 0);
...
exports.postShipmentToLots = postShipmentToLots;
/** Cost responsibility for a shipment built from a PO (BP-26 — was hardcoded "Marianna"). */
function responsibilityForPOShipment(po, supplierManagedTransport) {
    const term = String((po === null || po === void 0 ? void 0 : po.buyIncoterm) || "").toUpperCase();
    if (supplierManagedTransport || term === "DDP" || term === "DAP")
        return "Supplier";
    return "Marianna";
}
exports.responsibilityForPOShipment = responsibilityForPOShipment;
function nextShipmentNumberPure(shipments, year) {
    let max = 0;
    (shipments || []).forEach(s => {
...
exports.nextShipmentNumberPure = nextShipmentNumberPure;
...
function buildCollectionShipment(so, lots, shipments, info, deps) {
    var _a, _b;
    const year = (info === null || info === void 0 ? void 0 : info.year) || new Date(deps.todayISO()).getFullYear();
    const goods = (so.items || []).map((it) => {
        const lot = (0, salesOrders_domain_1.findLotForSOLine)(lots, it);
        return {
            id: deps.nextId(),
            poRef: it.sourceType === "PO" ? it.sourceRef : ((lot === null || lot === void 0 ? void 0 : lot.poRef) || ""),
            soRef: so.number,
            lotRef: (lot === null || lot === void 0 ? void 0 : lot.number) || (it.sourceType === "STOCK" ? it.sourceRef : ""),
    ...
    return {
        ...
        purpose: "OUTBOUND",
        status: "Booked",
        poRefs: Array.from(new Set(goods.map((g) => g.poRef).filter(Boolean))),
        soRefs: [so.number],
        lotRefs: Array.from(new Set(goods.map((g) => g.lotRef))),
        costResponsibility: "Client",
        supplierManagedTransport: false,
        originLocationId: (_a = firstLot === null || firstLot === void 0 ? void 0 : firstLot.locationId) !== null && _a !== void 0 ? _a : null,
        destinationCustom: `Client collection (EXW) — ${((_b = so.client) === null || _b === void 0 ? void 0 : _b.name) || ""}`.trim(),
        loadingDate: (info === null || info === void 0 ? void 0 : info.date) || deps.todayISO(),
        expectedDeliveryDate: (info === null || info === void 0 ? void 0 : info.date) || deps.todayISO(),
        collection: { date: (info === null || info === void 0 ? void 0 : info.date) || deps.todayISO(), truckPlate: (info === null || info === void 0 ? void 0 : info.truckPlate) || "", driverName: (info === null || info === void 0 ? void 0 : info.driverName) || "", notes: (info === null || info === void 0 ? void 0 : info.notes) || "" },
        legs: [], goods, costs: [],
        documents: [],
        notes: `EXW client collection recorded from ${so.number}.${(info === null || info === void 0 ? void 0 : info.notes) ? " " + info.notes : ""}`,
    };
}
exports.buildCollectionShipment = buildCollectionShipment;
...
function appendSourceGoods(sh, kind, doc, lots, deps) {
    const norm = (p) => String(p || "").toLowerCase().trim();
    const newGoods = (doc.items || []).map((it) => {
        const lot = kind === "SO"
            ? (0, salesOrders_domain_1.findLotForSOLine)(lots, it)
            : (lots || []).find(l => l.poRef === doc.number && norm(l.product) === norm(it.product)) || null;
        return {
            id: deps.nextId(),
            poRef: kind === "PO" ? doc.number : (it.sourceType === "PO" ? it.sourceRef : ((lot === null || lot === void 0 ? void 0 : lot.poRef) || "")),
            soRef: kind === "SO" ? doc.number : "",
            lotRef: (lot === null || lot === void 0 ? void 0 : lot.number) || (kind === "SO" && it.sourceType === "STOCK" ? it.sourceRef : ""),
            product: it.product, variety: it.variety || "", cnCode: it.cnCode || "",
            origin: it.origin, quality: it.quality, size: it.size, packaging: it.packaging,
            qtyKg: num(it.qty), pallets: num(it.pallets) || 0,
            description: `${it.product || "Goods"}${it.variety ? " " + it.variety : ""} ${it.packaging || ""}`.trim(),
    ...
    const poRefs = uniq([...(sh.poRefs || []), ...newGoods.map((g) => g.poRef)]);
    const soRefs = uniq([...(sh.soRefs || []), ...newGoods.map((g) => g.soRef)]);
    ...
    return {
        ...
        poRefs, soRefs,
        lotRefs: uniq([...(sh.lotRefs || []), ...newGoods.map((g) => g.lotRef)]),
    };
}
exports.appendSourceGoods = appendSourceGoods;
...
exports.SHIPMENT_LIFECYCLE = ["Draft", "Booked", "Loaded", "Delivered", "Closed"];
function canonicalStatus(s) {
    const v = String(s || "").trim();
    if (v === "Confirmed")
        return "Booked";
    if (v === "Arrived" || v === "In Transit")
        return "Loaded";
    if (v === "Planned")
        return "Draft";
    return v || "Draft";
}
exports.canonicalStatus = canonicalStatus;
/** The single next logical action for a shipment (BP-22) — not all buttons at once. */
function nextShipmentAction(sh) {
    const s = canonicalStatus(sh === null || sh === void 0 ? void 0 : sh.status);
...
exports.nextShipmentAction = nextShipmentAction;
...
function normalizeCustoms(raw) {
    ...
    return {
        applies: !notReq,
        role: notReq ? "not_required" : "",
        place: notReq ? "" : text,
        status: notReq ? "cleared" : "pending",
...
exports.normalizeCustoms = normalizeCustoms;
