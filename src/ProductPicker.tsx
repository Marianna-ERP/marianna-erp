...
export function ItemVarietyPicker({ catalog = [], setCatalog, item = "", variety = "", onItem, onVariety }: any) {
  ...
  const onItemChange = (val: string) => {
    if (val === "__add__") {
      const name = (window.prompt("New item name (added to the product catalog):") || "").trim();
      if (!name) return;
  ...
  const onVarChange = (val: string) => {
    if (val === "__add__") {
      const name = (window.prompt(`New variety for ${item || "item"} (added to the catalog):`) || "").trim();
      if (!name) return;
